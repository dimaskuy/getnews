...
// News Box
// fetch(`https://newsapi.org/v2/top-headlines?q=${"covid-19"}&country=id&pageSize=40&apiKey=b4e11073d4ad422690701532b24a3649`)
fetch(`https://newsapi.org/v2/everything?q=${"covid-19"}&language=id&pageSize=40&apiKey=b4e11073d4ad422690701532b24a3649`)
...
// Hot News
fetch("https://newsapi.org/v2/top-headlines?country=id&pageSize=38&apiKey=b4e11073d4ad422690701532b24a3649")
  .then((response) => response.json())
  .then((results) => {
...
categoryLi.forEach((category) => {
  category.addEventListener("click", function (e) {
    ...
    if (e.target.dataset.category === "all" && e.target.classList.contains("current")) {
      document.querySelector(".hot-news .title-info h2").textContent = `Berita Terkini dari Indonesia 🔥`;

      fetch("https://newsapi.org/v2/top-headlines?country=id&pageSize=38&apiKey=4a94eadbbdc44c87b1a53005a2ddc650")
        .then((response) => response.json())
        .then((results) => {
          checkNewsResults(results);
        });
    } else if (e.target.dataset.category !== "all" && e.target.classList.contains("current")) {
      document.querySelector(".hot-news .title-info h2").textContent = `Berita Indonesia Tekini dari Kategori ${e.target.textContent} 🔥`;

      fetch(`https://newsapi.org/v2/top-headlines?country=id&category=${e.target.dataset.category}&pageSize=38&apiKey=4a94eadbbdc44c87b1a53005a2ddc650`)
        .then((response) => response.json())
        .then((results) => {
          checkNewsResults(results);
        });
...
searchForm.forEach((form) => {
  form.addEventListener("submit", function (event) {
    ...
    searchNews.forEach((input) => {
      ...
      fetch(`https://newsapi.org/v2/everything?q=${input.value}&language=id&apiKey=b4e11073d4ad422690701532b24a3649`)
        .then((response) => response.json())
        .then((results) => {
          if (results.totalResults === results.articles.length) {
            document.querySelector(".totalResultsSearch").textContent = `Hasil Total untuk "${input.value}" : ${results.totalResults}.`;
          } else {
            document.querySelector(".totalResultsSearch").textContent = `Hasil Total untuk "${input.value}" : ${results.totalResults}. Hanya bisa menampilkan ${results.articles.length} hasil.`;
          }

          if (results.totalResults === 0) {
            console.log(results);
            document.querySelector(".totalResultsSearch").textContent = `Hasil Total untuk "${input.value}": 0`;
      ...
      // .catch(error => {
      //   document.querySelector('.totalResultsSearch').textContent = `Hasil Total untuk ${this.value}: 0`;
      //   newsSearchContainer.innerHTML = showErrorNewsSearch();
      // });
    });
  });
...
function checkNewsResults(results) {
  if (results.status === "ok") {
    document.querySelector(".totalResults").textContent = `Hasil Total: ${results.articles.length}`;
...
function showErrorNews() {
  return `<div class="row text-center error-msg mt-5">
            <div class="col-12">
              <h2 class="font-weight-bold mb-5">Whoops! Ada sesuatu yang salah disini. (Server Error)</h2>
...
function showNewsResults(response) {
  document.querySelector(".totalResultsSearch").textContent = `Hasil Total: ${response.totalResults}. Hanya bisa menampilkan ${response.articles.length}`;
}

function showNewsResultsSearch(response, keyword) {
  document.querySelector(".totalResultsSearch").textContent = `Hasil Total untuk ${keyword}: ${response.totalResults}. Hanya bisa menampilkan ${response.articles.length}`;
}

function showErrorNewsSearch() {
  return `<div class="row text-center error-msg mt-5">
            <div class="col-12">
              <h2 class="font-weight-bold mb-4">Tidak bisa menampilkan pencarian tersebut. Silahkan coba lagi, atau masukkan kata yang spesifik.</h2>
            </div>
          </div>`;
}
